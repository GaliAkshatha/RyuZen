...
function addToRemoveQueue(toastId: string, dispatchFn: (action: Action) => void) {
  ...
  const timeout = setTimeout(() => {
    ...
    dispatchFn({ type: actionTypes.REMOVE_TOAST, toastId });
  }, TOAST_REMOVE_DELAY);
...
function dispatch(action: Action) {
  ...
  if (action.type === actionTypes.DISMISS_TOAST) {
    const idsToQueue = action.toastId ? [action.toastId] : memoryState.toasts.map((t) => t.id);
    idsToQueue.forEach((id) => addToRemoveQueue(id, dispatch));
  }
}
