/**
 * Mirrors IOrganizationSettings exactly — the much richer, deeply
 * nested settings model (18 categories, ~70 leaf fields) exposed by
 * the separate ORG_ADMIN-only /organizations/settings endpoint.
 * Genuinely distinct from AD2's flat 11-boolean `settings` embedded
 * directly on the Organization entity itself.
 *
 * Confirmed this milestone: GET auto-creates and returns DEFAULT
 * settings if none exist yet for the caller's organization
 * (settingsRepository.createDefault) — never a 404 "not configured
 * yet" state to handle. Both GET and PATCH are ORG_ADMIN-only, no
 * :id param — always scoped to the caller's own organization.
 */
...
export type OrganizationSettingsCategory = Exclude<
  keyof OrganizationSettingsResponseDto,
  "id" | "organizationId" | "createdAt" | "updatedAt"
>;

/** Mirrors UpdateOrganizationSettingsDto — every category and every leaf field optional */
export type UpdateOrganizationSettingsPayload = {
  [K in OrganizationSettingsCategory]?: Partial<OrganizationSettingsResponseDto[K]>;
};
